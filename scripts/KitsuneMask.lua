...
---@return Vector3
local function dangleRightRotationPhysicsCorrection()
	return vec(0, 0, 0)
end

---@return Vector3
local function dangleLeftRotationPhysicsCorrection()
...
events.RENDER:register(function(delta, renderContext, matrix)
	...
	elseif MASK_STATE == 1 then
		...
		MASK_FOX.mask_fox.dangle_right:setRot(dangleRightRotationPhysicsCorrection())
		MASK_FOX.mask_fox.dangle_left:setRot(dangleLeftRotationPhysicsCorrection())
	-- Off to the side
	elseif MASK_STATE == 2 then
		MASK_FOX.mask_fox:setRot(OFFSET_ROTATION)
		MASK_FOX.mask_fox:setPos(OFFSET_POSITION)
		MASK_FOX.strap:setVisible(false)
		MASK_FOX.strap_offtotheside:setVisible(true)
		MASK_FOX.mask_fox:setVisible(true)
		MASK_FOX.mask_fox.dangle_right:setRot(DANGLE_OFFSET_ROTATION + dangleRightRotationPhysicsCorrection())
		MASK_FOX.mask_fox.dangle_left:setRot(DANGLE_OFFSET_ROTATION + dangleLeftRotationPhysicsCorrection())
	end
